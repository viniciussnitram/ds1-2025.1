...
export default function AlocarTurmaSala() {
  ...
  const handleEditPreferences = async (turma) => {
    try {
      ...
      const response = await axios.get(
        `http://localhost:5000/api/Disciplina/${disciplinaId}`
      );
  ...
  const handleSavePreferences = async () => {
    try {
      ...
      await axios.put(
        `http://localhost:5000/api/Disciplina/${selectedDisciplina.id}`,
        payload
      );
  ...
  const handleBuscarAlocacoes = async () => {
    try {
      ...
      const response = await axios.get(
        `http://localhost:5000/api/Sala/${selectedSalaId}`
      );
      const alocacoes = response.data.alocacoes || [];
